...
class UploadFileRequestHandler{
    ...
    class func uploadFile(chat:Chat,
                          req:NewUploadFileRequest,
        ...
        let url = SERVICE_ADDRESSES_ENUM().PODSPACE_FILESERVER_ADDRESS + filePath.rawValue
        ...
        UploadManager.upload(url: url,
            ...
            if let data = data , let chatError = try? JSONDecoder().decode(ChatError.self, from: data) , chatError.hasError == true{
                ...
                let fileDetail   = FileDetail(fileExtension: req.fileExtension, link: link, mimeType: req.mimeType, name: req.fileName, originalName: req.originalName, size: req.fileSize)
