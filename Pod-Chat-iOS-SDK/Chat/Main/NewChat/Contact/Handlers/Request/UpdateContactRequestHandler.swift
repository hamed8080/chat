...
class UpdateContactRequestHandler {
    ...
    class func handle(_ req:UpdateContactRequest,
		...
		let url = "\(config.platformHost)\(SERVICES_PATH.UPDATE_CONTACTS.rawValue)"
		let headers: HTTPHeaders = ["_token_": config.token, "_token_issuer_": "1"]
		chat.restApiRequest(req, decodeType: NewContactResponse.self, url: url,bodyParameter: true, method: .post ,headers: headers , uniqueIdResult: uniqueIdResult){ response in
	...
	private class func insertOrUpdateCache(chat:Chat , contactsResponse:NewContactResponse?){
