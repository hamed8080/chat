        ...
        let task = session.dataTask(with: urlRequest) { data, response, error in
            ...
            DispatchQueue.main.async {
                if let data = data , let chatError = try? JSONDecoder().decode(ChatError.self, from: data) , chatError.hasError == true{
                    ...
                    let error = ChatError(message: "\(ChatErrorCodes.NETWORK_ERROR.rawValue) \(error)", errorCode: httpResponse.statusCode, hasError: true)
                    completion(nil,error)
                }else {
                    let error = ChatError(message: "\(ChatErrorCodes.NETWORK_ERROR.rawValue)", errorCode: httpResponse.statusCode, hasError: true)
                    completion(nil,error)
                }
            }
        }
