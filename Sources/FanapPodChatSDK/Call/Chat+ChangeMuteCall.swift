...
// Request
extension Chat {
    func requestMuteCall(_ req: MuteCallRequest, _ completion: CompletionType<[CallParticipant]>? = nil, _ uniqueIdResult: UniqueIdResultType? = nil) {
        prepareToSendAsync(req: req, uniqueIdResult: uniqueIdResult, completion: completion)
        webrtc?.toggle()
    }

    func requestUNMuteCall(_ req: UNMuteCallRequest, _ completion: CompletionType<[CallParticipant]>? = nil, _ uniqueIdResult: UniqueIdResultType? = nil) {
        prepareToSendAsync(req: req, uniqueIdResult: uniqueIdResult, completion: completion)
        webrtc?.toggle()
    }
}
...
extension Chat {
    func onMuteChanged(_ asyncMessage: AsyncMessage) {
        ...
        delegate?.chatEvent(event: chatMessage.type == .muteCallParticipant ? .call(.callParticipantMute(callParticipants)) : .call(.callParticipantUnmute(callParticipants)))
