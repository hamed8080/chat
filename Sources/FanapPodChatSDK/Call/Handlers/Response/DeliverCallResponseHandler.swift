...
class DeliverCallResponseHandler {
    static func handle(_ asyncMessage: AsyncMessage) {
        ...
        chat.delegate?.chatEvent(event: .call(CallEventModel(type: .callDelivered(call))))

        guard let callback = chat.callbacksManager.getCallBack(chatMessage.uniqueId) else { return }
        callback(.init(uniqueId: chatMessage.uniqueId, result: call))
        chat.callbacksManager.removeCallback(uniqueId: chatMessage.uniqueId, requestType: .deliveredCallRequest)
    }
}
