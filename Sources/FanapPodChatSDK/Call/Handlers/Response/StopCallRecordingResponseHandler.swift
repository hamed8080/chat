...
class StopCallRecordingResponseHandler {
    static func handle(_ asyncMessage: AsyncMessage) {
        ...
        chat.delegate?.chatEvent(event: .call(CallEventModel(type: .stopCallRecording(participant))))

        guard let callback = chat.callbacksManager.getCallBack(chatMessage.uniqueId) else { return }
        callback(.init(uniqueId: chatMessage.uniqueId, result: participant))
        chat.callbacksManager.removeCallback(uniqueId: chatMessage.uniqueId, requestType: .stopRecording)
    }
}
