...
class UNMuteCallParticipantResponseHandler {
    static func handle(_ asyncMessage: AsyncMessage) {
        ...
        chat.delegate?.chatEvent(event: .call(.callParticipantUnmute(callParticipants)))

        guard let callback = chat.callbacksManager.getCallBack(chatMessage.uniqueId) else { return }
        callback(.init(uniqueId: chatMessage.uniqueId, result: callParticipants))
        chat.callbacksManager.removeCallback(uniqueId: chatMessage.uniqueId, requestType: .unmuteCallParticipant)
    }
}
