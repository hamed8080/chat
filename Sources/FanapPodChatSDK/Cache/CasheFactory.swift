...
//        case pinMessage(_ pinMessage: PinMessage, _ threadId: Int?)
//        case unpinMessage(_ unpinMessage: PinMessage, _ threadId: Int?)
...
//            let contacts = Contact.getContacts(req: req)
...
//            let threads = Conversation.getThreads(req: req)
...
//            let cmParticipants = Participant.fetchWithOffset(count: req.count, offset: req.offset, predicate: predicate)
...
//            Contact.insertOrUpdate(contacts: contacts)
//        case .getBlockedUsers:
//            break
//        case let .threads(_: threads):
//            Conversation.insertOrUpdate(conversations: threads)
//        case let .participants(_: participants, _: threadId):
//            Participant.insertOrUpdateParicipants(participants: participants, fromMainMethod: true, threadId: threadId)
//        case let .removeParticipants(_: participants, _: threadId):
//            Participant.deleteParticipants(participants: participants, threadId: threadId)
//        case let .leaveThread(_: participantId):
//            Participant.crud.deleteWith(predicate: NSPredicate(format: "id == %i", participantId), logger)
...
//            User.insertOrUpdate(user: user, resultEntity: nil)
//        case let .pinMessage(_: pinMessage, _: threadId):
//            PinMessage.pinMessage(pinMessage: pinMessage, threadId: threadId)
//        case let .unpinMessage(_: unpinMessage, _: threadId):
//            PinMessage.unpinMessage(pinMessage: unpinMessage, threadId: threadId)
//        case let .clearAllHistory(_: threadId):
//            Message.crud.deleteWith(predicate: NSPredicate(format: "conversation.id == %i OR threadId == %i", threadId, threadId), logger)
//        case let .deleteMessage(_: threadId, _: messageId):
//            Message.crud.deleteEntityWithPredicate(predicate: NSPredicate(format: "conversation.id == %i OR threadId == %i AND id == %i", threadId, threadId, messageId))
//        case let .deleteMessages(_: threadId, _: messageIds):
//            Message.crud.fetchWith(NSPredicate(format: "conversation.id == %i OR threadId == %i AND id IN %@", threadId, threadId, messageIds))?.forEach { entity in
...
//            Message.insertOrUpdate(message: message)
...
//            QueueOfFileMessages.crud.deleteWith(predicate: NSPredicate(format: "uniqueId == %@", uniqueId), logger)
//        case .deleteAllUser:
//            User.crud.deleteAll(logger)
//        case let .deleteContacts(_: contactIds):
//            Contact.crud.deleteWith(predicate: NSPredicate(format: "id IN %@", contactIds), logger)
//        case let .deleteThreads(_: threadIds):
//            Conversation.crud.deleteWith(predicate: NSPredicate(format: "id IN %@", threadIds), logger)
//        case .deleteAllContacts:
//            Contact.crud.deleteAll(logger)
//        case .deleteAllThreads:
//        Conversation.crud.deleteAll(logger)
//        case .deleteAllMessage:
