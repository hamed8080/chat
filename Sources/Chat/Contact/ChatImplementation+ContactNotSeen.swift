//
// ChatImplementation+ContactNotSeen.swift
...
public extension ChatImplementation {
    ...
    func contactNotSeen(_ request: NotSeenDurationRequest, completion: @escaping CompletionType<[UserLastSeenDuration]>, uniqueIdResult: UniqueIdResultType? = nil) {
        prepareToSendAsync(req: request, type: .getNotSeenDuration, uniqueIdResult: uniqueIdResult) { (response: ChatResponse<ContactNotSeenDurationRespoonse>) in
            completion(ChatResponse(uniqueId: request.chatUniqueId, result: response.result?.notSeenDuration, error: response.error))
        }
    }
}
