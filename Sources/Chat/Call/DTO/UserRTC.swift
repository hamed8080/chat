...
public protocol UserRTCProtocol {
    ...
    func setRemoteIce(_ rtcIce: RTCIceCandidate)
    func close()
}
...
@ChatGlobalActor
public class AudioRTC: UserRTC, AudioRTCProtocol, @unchecked Sendable {
    ...
    init(direction: RTCDirection, topic: String, config: WebRTCConfig, delegate: RTCPeerConnectionDelegate) {
        ...
        Task {
            if TARGET_OS_SIMULATOR != 0 {
                self.renderer = await RTCEAGLVideoView(frame: .zero)
            } else {
                self.renderer = await RTCMTLVideoView(frame: .zero)
            }
        }
    }
